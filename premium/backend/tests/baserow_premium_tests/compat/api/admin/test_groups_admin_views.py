...
def test_list_admin_workspaces(
    ...
    workspace_1.save()
    workspace_2 = premium_data_fixture.create_workspace(
    ...
    response = api_client.get(
        f'{reverse("api:premium:admin:groups:list")}?search={workspace_1.name}',
        format="json",
        HTTP_AUTHORIZATION=f"JWT {staff_token}",
    )
    response_json = response.json()
...
def test_delete_workspace(api_client, premium_data_fixture):
    ...
    url = reverse("api:premium:admin:groups:edit", kwargs={"group_id": workspace.id})
