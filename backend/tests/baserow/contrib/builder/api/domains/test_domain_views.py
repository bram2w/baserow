...
def test_create_domain_only_one_domain_allowed(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    builder = data_fixture.create_builder_application(user=user)

    data_fixture.create_builder_domain(builder=builder, domain_name="test.com")
    ...
    assert response.status_code == HTTP_400_BAD_REQUEST
    assert response.json()["error"] == "ERROR_ONLY_ONE_DOMAIN_ALLOWED"
