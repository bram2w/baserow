...
@pytest.mark.django_db
def test_link_element_path_parameter_wrong_type(data_fixture):
    builder = data_fixture.create_builder_application()
    page = data_fixture.create_builder_page(builder=builder)
    page_with_params = data_fixture.create_builder_page(
        builder=builder,
    ...
    with pytest.raises(ValidationError):
        ElementHandler().update_element(
            link_element, page_parameters=[{"name": "id", "value": "not a number"}]
        )

    try:
        ElementHandler().update_element(
            link_element, page_parameters=[{"name": "id", "value": None}]
        )
    except Exception as e:
        pytest.fail(f"Should have not raised an error, {e}")
