...
def test_list_applications(
    api_client, data_fixture, django_assert_num_queries, group_compat_timebomb
):
    ...
    with patch(
        ...
        response = api_client.get(
            reverse(
                "api:applications_compat:list", kwargs={"group_id": workspace_1.id}
            ),
    ...
    response = api_client.get(
        reverse("api:applications_compat:list", kwargs={"group_id": workspace_2.id}),
        **{"HTTP_AUTHORIZATION": f"JWT {token}"},
    )
    ...
    response = api_client.get(
        reverse("api:applications_compat:list", kwargs={"group_id": 999999}),
        **{"HTTP_AUTHORIZATION": f"JWT {token}"},
    )
    assert response.status_code == HTTP_404_NOT_FOUND
    assert response.json()["error"] == "ERROR_GROUP_DOES_NOT_EXIST"

    url = reverse("api:applications_compat:list", kwargs={"group_id": workspace_1.id})
    ...
    response = api_client.delete(
        reverse("api:groups:item", kwargs={"group_id": workspace_1.id}),
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    ...
    for application in response.json():
        assert application["group"]["id"] != workspace_1.id
...
@pytest.mark.django_db
@pytest.mark.disabled_in_ci
def test_create_application(api_client, data_fixture, group_compat_timebomb):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    workspace_2 = data_fixture.create_workspace()

    response = api_client.post(
        reverse("api:applications_compat:list", kwargs={"group_id": workspace.id}),
    ...
    url = reverse("api:applications_compat:list", kwargs={"group_id": workspace_2.id})
    response = api_client.get(url)
    assert response.status_code == HTTP_401_UNAUTHORIZED
...
@pytest.mark.django_db
@pytest.mark.disabled_in_ci
def test_get_application(api_client, data_fixture, group_compat_timebomb):
    user, token = data_fixture.create_user_and_token()
    ...
    assert response_json["group"]["id"] == workspace.id

    response = api_client.delete(
        reverse("api:groups:item", kwargs={"group_id": application.workspace.id}),
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_204_NO_CONTENT
...
@pytest.mark.django_db
@pytest.mark.disabled_in_ci
def test_update_application(api_client, data_fixture, group_compat_timebomb):
    user, token = data_fixture.create_user_and_token()
    user_2, token_2 = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    workspace_2 = data_fixture.create_workspace(user=user_2)
...
@pytest.mark.django_db
@pytest.mark.disabled_in_ci
def test_delete_application(api_client, data_fixture, group_compat_timebomb):
    user, token = data_fixture.create_user_and_token()
    user_2, token_2 = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    workspace_2 = data_fixture.create_workspace(user=user_2)
...
@pytest.mark.django_db
@pytest.mark.disabled_in_ci
def test_order_applications(api_client, data_fixture, group_compat_timebomb):
    user, token = data_fixture.create_user_and_token(
        email="test@test.nl", password="password", first_name="Test1"
    )
    workspace_1 = data_fixture.create_workspace(user=user)
    ...
    response = api_client.post(
        reverse("api:applications_compat:order", kwargs={"group_id": workspace_2.id}),
        {"application_ids": []},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    ...
    response = api_client.post(
        reverse("api:applications_compat:order", kwargs={"group_id": 999999}),
        {"application_ids": []},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    ...
    response = api_client.post(
        reverse("api:applications_compat:order", kwargs={"group_id": workspace_1.id}),
        {"application_ids": [0]},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
...
@pytest.mark.django_db
@pytest.mark.disabled_in_ci
def test_duplicate_application_errors(api_client, data_fixture, group_compat_timebomb):
...
def test_duplicate_application_schedule_job(
    mock_run_async_job, api_client, data_fixture, group_compat_timebomb
):
