...
def test_can_undo_an_action_and_get_correct_response_code(api_client, data_fixture):
    ...
    assert Workspace.objects.filter(id=group_id).exists()
    ...
    assert not Workspace.objects.filter(id=group_id).exists()
    assert response.status_code == HTTP_200_OK
    assert response.json() == {
        "actions": [
            {
...
def test_undoing_an_action_which_fails_returns_correct_result_code(
    ...
    Workspace.objects.filter(id=group_id).delete()
...
def test_redoing_an_action_which_fails_returns_correct_result_code(
    ...
    Workspace.objects_and_trash.filter(id=group_id).delete()
...
def test_can_undo_an_action_group_and_get_correct_response_code(
    ...
    assert Workspace.objects.filter(id=group1_id).exists()
    ...
    assert Workspace.objects.filter(id=group2_id).exists()
    ...
    assert not Workspace.objects.filter(id__in=[group1_id, group2_id]).exists()
    assert response.status_code == HTTP_200_OK
    assert response.json() == {
        "actions": [
            {
