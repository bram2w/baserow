...
def test_list_templates(api_client, data_fixture):
    ...
    response = api_client.get(reverse("api:templates_compat:list"))
    ...
    assert response_json[0]["templates"][0]["group_id"] == template_1.workspace_id
...
def test_install_template(api_client, data_fixture):
    ...
    response = api_client.post(
        reverse(
            "api:templates_compat:install",
            kwargs={"group_id": workspace.id, "template_id": template_2.id},
        ),
    ...
    response = api_client.post(
        reverse(
            "api:templates_compat:install",
            kwargs={"group_id": workspace_2.id, "template_id": template.id},
        ),
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_400_BAD_REQUEST
    ...
    response = api_client.post(
        reverse(
            "api:templates_compat:install",
            kwargs={"group_id": workspace.id, "template_id": 0},
        ),
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_404_NOT_FOUND
    ...
    response = api_client.post(
        reverse(
            "api:templates_compat:install",
            kwargs={"group_id": workspace.id, "template_id": template.id},
        ),
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_200_OK
    ...
    assert response_json[0]["group"]["id"] == workspace.id
    application = Application.objects.all().order_by("id").last()
    assert response_json[0]["id"] == application.id
    assert response_json[0]["group"]["id"] == application.workspace_id
...
def test_async_install_template_errors(api_client, data_fixture):
    ...
    response = api_client.post(
        reverse(
            "api:templates_compat:install_async",
            kwargs={"group_id": workspace.id, "template_id": template_2.id},
        ),
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
...
def test_async_install_template_serializer(api_client, data_fixture):
    ...
    assert job_rsp["group"]["id"] == workspace.id
    ...
    assert installed_app["group"]["id"] == workspace.id
