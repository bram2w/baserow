...
```bash
docker run -v baserow_data:/baserow/data -p 80:80 -p 443:443 baserow/baserow:1.17.1
```
...
docker run -it --rm -v baserow_data:/baserow/data baserow/baserow:1.17.1 \
...
docker run -it --rm -v baserow_data:/baserow/data -v $PWD:/baserow/host \
   baserow/baserow:1.17.1 backend-cmd-with-db backup -f /baserow/host/backup.tar.gz
```
