...
```bash
docker run -v baserow_data:/baserow/data -p 80:80 -p 443:443 baserow/baserow:1.26.1
```
...
The `baserow/baserow:1.26.1` image by default runs all of Baserow's various services in
...
docker run \
  ...
  --restart no \
  baserow/baserow-pgautoupgrade:1.26.1
```
...
docker run -it --rm -v baserow_data:/baserow/data baserow/baserow:1.26.1 \
...
docker run -it --rm -v baserow_data:/baserow/data -v $PWD:/baserow/host \
   baserow/baserow:1.26.1 backend-cmd-with-db backup -f /baserow/host/backup.tar.gz
```
...
docker run -it --rm \
  ...
  baserow/baserow:1.26.1 backend-cmd-with-db restore -f /baserow/old_data/backup.tar.gz

# Or to restore from a file on your host instead run something like:
docker run -it --rm \
  -v baserow_data:/baserow/data -v \
  $(pwd):/baserow/host \
  baserow/baserow:1.26.1 backend-cmd-with-db restore -f /baserow/host/backup.tar.gz
```
