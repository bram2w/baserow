...
export BASEROW_EMAIL_RECEIVER_STARTUP_COMMAND="${BASEROW_EMAIL_RECEIVER_STARTUP_COMMAND:-email-receiver}"
# In the all-in-one image the backend runs in the same container, so the email
# receiver webhooks to localhost by default.
export BASEROW_INBOUND_EMAIL_WEBHOOK_URL="${BASEROW_INBOUND_EMAIL_WEBHOOK_URL:-http://localhost:8000/api/inbound-email/}"
