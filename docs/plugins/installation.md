...
6. Now build your custom Baserow with the plugin installed by running:
   `docker build -t my-customized-baserow:1.33.4 .`
...
it, the new container is created from the `baserow/baserow:1.33.4` image which does not
...
Baserow also provides `baserow/backend:1.33.4` and `baserow/web-frontend:1.33.4` images
...
6. Rebuild your image - `docker build -t my-customized-baserow:1.33.4 .`
