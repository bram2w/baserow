...
6. Now build your custom Baserow with the plugin installed by running:
   `docker build -t my-customized-baserow:1.27.1 .`
7. Finally, you can run your new customized image just like the normal Baserow image:
   `docker run -p 80:80 -v baserow_data:/baserow/data my-customized-baserow:1.27.1`
...
it, the new container is created from the `baserow/baserow:1.27.1` image which does not
...
Baserow also provides `baserow/backend:1.27.1` and `baserow/web-frontend:1.27.1` images
...
```
docker run --rm baserow/backend:1.27.1 install-plugin ... 
docker run -e BASEROW_PLUGIN_GIT_REPOS=https://example.com/example/plugin1.git,https://example.com/example/plugin2.git --rm baserow/backend:1.27.1
```
...
3. `docker run --rm -v baserow_data:/baserow/data baserow:1.27.1 uninstall-plugin plugin_name`
...
6. Rebuild your image - `docker build -t my-customized-baserow:1.27.1 .`
...
2. To uninstall a plugin you installed using one of `BASEROW_PLUGIN_GIT_REPOS`
   ...
   must:
    ...
    2. `docker run --rm -v baserow_data:/baserow/data baserow:1.27.1 uninstall-plugin plugin_name`
