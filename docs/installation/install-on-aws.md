...
The `baserow/baserow:1.34.3` image runs all of Baserow’s various services inside the
...
* Simpler to use and setup compared to the one container per-service model below
  because:
    ...
    * Configuring load balancers is easier as you can just directly route through all
      requests to any horizontally scaled container running `baserow/baserow:1.34.3`.
...
* Potentially higher resource usage overall as each of the all-in-one containers will
  ...
  specific services.
    * For example if you deploy 10 `baserow/baserow:1.34.3` containers horizontally you
...
When setting up the health check for the ALB the `baserow/baserow:1.34.3` container
...
Now we are ready to spin up our `baserow/baserow:1.34.3` containers. See below for a
...
1. Select the `baserow/baserow:1.34.3` image.
...
The `baserow/backend:1.34.3` and `baserow/web-frontend:1.34.3` images allow you to run
...
1. In the task defintion use the `baserow/backend:1.34.3` image
...
1. Use the `baserow/backend:1.34.3`
...
1. Use the `baserow/backend:1.34.3` image with `celery-worker` as the image command.
...
1. Use the `baserow/backend:1.34.3` image.
...
1. Use the `baserow/web-frontend:1.34.3` image with no arguments needed.
