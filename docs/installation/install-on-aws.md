...
The `baserow/baserow:1.21.0` image runs all of Baserow’s various services inside the
...
* Simpler to use and setup compared to the one container per-service model below
  because:
    ...
    * Configuring load balancers is easier as you can just directly route through all
      requests to any horizontally scaled container running `baserow/baserow:1.21.0`.
...
* Potentially higher resource usage overall as each of the all-in-one containers will
  ...
  specific services.
    * For example if you deploy 10 `baserow/baserow:1.21.0` containers horizontally you
...
When setting up the health check for the ALB the `baserow/baserow:1.21.0` container
...
Now we are ready to spin up our `baserow/baserow:1.21.0` containers. See below for a
...
1. Select the `baserow/baserow:1.21.0` image.
...
The `baserow/backend:1.21.0` and `baserow/web-frontend:1.21.0` images allow you to run
...
1. In the task defintion use the `baserow/backend:1.21.0` image
...
1. Use the `baserow/backend:1.21.0`
...
1. Use the `baserow/backend:1.21.0` image with `celery-worker` as the image command.
...
1. Use the `baserow/backend:1.21.0` image.
...
1. Use the `baserow/web-frontend:1.21.0` image with no arguments needed.
