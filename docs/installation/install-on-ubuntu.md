...
newgrp docker 
# Change BASEROW_PUBLIC_URLS to your domain name or http://YOUR_SERVERS_IP if you want
# to access Baserow remotely.
# This command will run Baserow with it's data stored in the new baserow_data docker 
# volume.
docker run -e BASEROW_PUBLIC_URLS=http://localhost \
--name baserow \
-d \
...
docker run \
  -d \
  --name baserow \
  -e BASEROW_PUBLIC_URLS=http://localhost \
