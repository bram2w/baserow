...
* `baserow/backend:1.25.1` which by default starts the Gunicorn Django backend server 
...
* `baserow/backend:1.25.1` (default command is `gunicorn`)
* `baserow/backend:1.25.1` with command `celery-worker`
* `baserow/backend:1.25.1` with command `celery-export-worker`
* `baserow/web-frontend:1.25.1` (default command is `nuxt-local`)
* A postgres database 
* A redis server
