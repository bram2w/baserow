...
* `baserow/backend:1.21.2` which by default starts the Gunicorn Django backend server 
  for Baserow but is also used to start the celery workers and celery beat services.
* `baserow/web-frontend:1.21.2` which is a Nuxt server providing Server Side rendering 
  for the website.
...
* `baserow/backend:1.21.2` (default command is `gunicorn`)
* `baserow/backend:1.21.2` with command `celery-worker`
* `baserow/backend:1.21.2` with command `celery-export-worker`
* `baserow/web-frontend:1.21.2` (default command is `nuxt-local`)
* A postgres database 
* A redis server
